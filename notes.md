...
49. Добавлен глагол hyr (искать) — базовый корень, от которого образовано hyrv (поиск) в компьютерной лексике
50. Добавлен глагол stev (остановить) — базовый корень для действия прекращения:
    - stev = остановить (переходный)
    - qe-stev = остановиться (возвратный, с префиксом qe-)
    - stev-op = остановите! (императив)
51. Добавлен глагол vozn (знакомить) — базовый корень:
    - vozn = знакомить (переходный)
    - qe-vozn = знакомиться (возвратный)
52. Добавлен глагол mozv (встречать) — базовый корень:
    - mozv = встречать
    - mozvtel = встречающий (причастие настоящего)
    - imozvtel = встречавший (причастие прошедшего, активное)
53. Добавлен глагол seŋv (следовать) и наречие seŋvik (следом)
54. Добавлена временная лексика:
    - ryŋv (рассвет), syŋv (закат), ryŋgylt (раннее утро при рассвете)
    - vortek (раньше), prem (прежде)
55. Добавлены абстрактные понятия:
    - noŋm (имя), treŋ (нужда, необходимость)
56. Добавлено вопросительное слово qeŋ (зачем) — отличается от qevr (почему):
    - qevr = почему (причина)
    - qeŋ = зачем (цель)
57. Добавлено наречие trevik (истинно) от trev (истина) + -ik
58. Добавлены сложные понятия:
    - ynvelk (с первого взгляда) = yn (один) + velk (видеть)
    - tolmvelk (переосмысление) = tolm (думать) + velk (видеть)
59. Добавлен глагол pyrg (вести за поводья) — для ведения животных
60. Глагол qe-rhep (влюбиться) образуется от rhep (любить) с возвратным префиксом qe-
61. МАССОВАЯ ЗАМЕНА ЗАИМСТВОВАНИЙ (Ревизия 12):
    Все слова, похожие на заимствования из других языков, заменены на оригинальные корни.
    Примеры замен:
    - Глаголы: qolb (остановить), tymk (следовать), pyrg (вести), gopk (лежать), rymk (сидеть), volb (стоять)
    - Звуки: zelk (крик), glorm (шум), rymbel (полусидя)
    - Свойства: beltq (горький), qoplex (мягкий), gorlex (твёрдый), velmok (гладкий)
    - Химия: melqv (металл), xyrk (кремний), zepyl (золото), bylrv (серебро), felzv (железо)
    - Физика: fyglv (газ), zolqv (жидкость), gorlv (твёрдое), pylzv (плазма)
    - География: kelvn (город), broxn (село), mylpn (деревня), qyrbn (ауыл)
62. Стороны света образованы от природных явлений:
    - qyvzev (восток) = qyv (солнце) + zev (к) — туда, где восходит солнце
    - nyphzev (запад) = nyph (луна) + zev (к) — туда, где появляется луна
    - varmzev (юг) = varm (тепло) + zev (к) — туда, где тепло
    - lekzev (север) = lek (холод) + zev (к) — туда, где холодно