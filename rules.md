...
**Всего фонем: 26 (7 гласных, 19 согласных)**
...
|-------|--------------|---------------|
| NOM | mor | mor-yn |
| ACC | mor-em | mor-em-yn |
| DAT | mor-al | mor-al-yn |
| INS | mor-ev | mor-ev-yn |
| LOC | mor-ok | mor-ok-yn |
...
**Правила выбора аспекта:**

| Аспект | Когда использовать | Коммуникативный фокус |
|--------|-------------------|----------------------|
| Нейтральный (∅) | Констатация факта, общее утверждение | "Это происходит" |
| Прогрессив (-en) | Действие в процессе, занятость | "Сейчас занят этим" |
| Хабитуалис (-yr) | Регулярность, расписание, привычка | "Так бывает обычно" |

**Контрастные примеры:**

```
qa vek vyg = "я работаю" (констатация факта, общее утверждение)
qa veken vyg = "я работаю" (не мешай, сейчас занят)
qa vekyr vyg utyr = "я работаю завтра" (по расписанию, регулярно)

qa tul = "я иду" (факт перемещения)
qa tulen = "я иду" (в процессе, прямо сейчас двигаюсь)
qa tulyr kevok = "я хожу домой" (регулярно, каждый день)
```
